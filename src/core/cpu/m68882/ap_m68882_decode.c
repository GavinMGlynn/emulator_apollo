...
/* **Which** defined operation a redundant encoding is redundant *with*.
 *
 * Footnote 3 says only that these are "redundant with valid instructions
 * implemented by the FPCP" and do not trap. It never names the instruction, so
 * this used to decline -- and declining is what made a reserved encoding report
 * a gap in this model, which it is not: the part executes them.
 *
 * The mapping is read off the decode rather than invented, and every group has
 * corroboration:
 *
 *   `$28-$2F` -> `FSUB`.       Bits 2-0 ignored. The oracle groups all eight
 *                              onto one arm, and `$28` is the only defined
 *                              encoding in the range.
 *   `$38-$3F` -> `FCMP`/`FTST`. Bits 0 and 2 ignored, **bit 1 selects**: the
 *                              oracle groups `$38 $39 $3C $3D` and
 *                              `$3A $3B $3E $3F`, which is exactly that rule,
 *                              and the two defined encodings `$38` and `$3A`
 *                              differ in bit 1 alone.
 *   `$05 $07 $0B $13 $17 $1B`  Bit 0 ignored. Each is an **odd** encoding whose
 *                              even neighbour is defined -- `FSQRT`,
 *                              `FLOGNP1`, `FATAN`, `FTENTOX`, `FLOG2`, `FNEG`
 *                              -- which is the same bit-0-ignored rule the
 *                              `$38` group demonstrates.
 *
 * **`PROVISIONAL` for those last six only.** The oracle declines them (it stops
 * on an unimplemented opmode), so the rule is carried over from the groups where
 * it is corroborated rather than observed on them directly. No assembler emits
 * these -- footnote 3 says they "should not be generated by assemblers or
 * compilers" -- so nothing measured depends on the choice; what matters is that
 * the machine executes rather than stopping. */
static unsigned redundant_alias(unsigned extension) {
  if (extension >= 0x28u && extension <= 0x2Fu) {
    return 0x28u; /* FSUB */
  }
  if (extension >= 0x38u && extension <= 0x3Fu) {
    /* Bit 1 alone survives: `$38` FCMP against `$3A` FTST. */
    return 0x38u | (extension & 0x02u);
  }
  /* Bit 0 ignored. */
  return extension & ~1u;
}
...
ap_m68882_command_word_t ap_m68882_decode_command(uint16_t word) {
  ...
  if (defined_operation(out.extension, &out.operation)) {
    ...
    /* And it carries the operation it aliases, so every caller downstream sees
     * a defined operation and the part executes what the hardware executes. */
    if (!defined_operation(redundant_alias(out.extension), &out.operation)) {
      /* Unreachable while the table above and `redundant_alias` agree, and
       * asserted by `fpu_suite` rather than left to trust. */
