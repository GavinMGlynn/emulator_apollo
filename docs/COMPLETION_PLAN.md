...
- [ ] **The boot's new verification, exercised: a framebuffer PNG.**
      `--screen c8p --screenshot FILE` captures the display, and the display
      **works**: a booting machine renders the PROM's self-test text, `SELF
      TESTS PASSED.` and `Domain/OS kernel(7), revision 10.4` in the firmware's
      own font. The black-framebuffer, silent-console and changed-boot-path
