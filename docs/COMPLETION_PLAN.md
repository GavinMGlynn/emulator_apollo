...
- [x] Two nodes see each other over the ring under Domain/OS -- **done
      2026-09-08**. Two booted Domain/OS nodes on one segment, each driven to a
      shell through its own Mnemonic Debugger, exchange ring frames and each
      reports the other: `node 0 claims 71 frames seen 104 copied 33 forced 0`
      and `node 1 claims 52 seen 124 copied 71`, with each node's received
      frames carrying **the other's ID** at offset 8 and each reading
      `ack 4A cpd icopy` back off its own returning frame -- §2.2.2.5, so both
      ends say a frame crossed. Eleven defects of ours were found and fixed
      getting there, three of them by the ring firmware's own self-test.
      *Verification: the counters and the transmit read-back above; two runs on
      their own disk pairs byte-identical; ring ROM self-test byte-identical
      (`d0 0`, 7,263,778 steps), identity boot `FE2BB02AEF1F4624` unchanged.
      Detail in `PROJECT_STATUS.md`; `FINDINGS.md` C243-C252.*
      *`/com/lcnode` still answers "No other nodes responded" while the frames
      cross, are copied and are acknowledged: what is missing is a **reply**,
      which is Domain/OS protocol and is the multi-node workloads item's.*
...
- [x] **Cross-check `ap_kbd.c`'s code table against `002398-03`'s keyboard
      charts.** Done 2026-09-13. p. 6-10, the Low-Profile chart in *physical*
      form, is p. 6-13's map printed by legend instead of by key number, and
      **it settles all eight defects p. 6-13 had left**: `C7`'s impossible `AB`
      is `A8`, and the two `PROVISIONAL` cells are `D14` control (`^\`) and
      `D13` control (`CRC`), not `D12`'s and `D11`'s — which Table 12-1's own
      `control = unshifted + 0x30` rule, obeyed by eleven other C-block keys,
      independently requires. Four table rows corrected. It also confirms the
      two pointing-device escapes (`DF` `mous`, `E8` `tpad` — so the
      absolute-mode device is the touchpad) and the bracket swap, and shows
      eleven cells that differ between the two *keyboards* rather than the two
      documents. *Verification: `kbd_suite` 62 → 69, p. 6-10 transcribed and
...
- **SCSI**: all five deliverables landed (`d9d5fcd8` bus, `2231d461` DMA,
  ...
  `APOLLO_DISK=$APOLLO_SCRATCH/ds5500-scsi.awd tools/dn5500-boot.sh --clock 2002-11-28T12:30:00 --scsi --scsi-drive`
...
**2. The 68040 item — closed 2026-09-15.** The fill landed: fetches and
operands go through the 68040's physically tagged caches, table searches read
through the data cache, and `CPUSH` writes dirty data back — missing since
2026-09-13, and the cause of the first boot on the fill crashing. SC1/SC0 and
TBI are read from `[040DH]` §7's reference adapter, `PROVISIONAL` for Apollo's
board. Detail in `PROJECT_STATUS.md`. *(Until then: "The 68040 item is one live
sub-item, and its title is stale ... attach them, so `CINV`/`CPUSH` stop being
no-ops and fetches and operands go through the caches".)*

**3. The Domain/OS software shelf — closed 2026-09-15.** Every one of the 100
documents has a recorded basis, verified by matching filenames against the
records; eleven are walked whole and none has chapters owed. Detail in
`PROJECT_STATUS.md`. *(Until then: "~80 untouched documents, seven part-read
records naming their own owed chapters, and the 805-page release-notes shelf.
Finishable; the only question is pace.")*
...
- [x] **The Domain/OS software shelf is read: every one of the 100 shelf
      documents has a recorded basis.** `docs/references/SOFTWARE_SHELF_WALK.md`
      is the register, checked by matching all 100 filenames against the walk and
      triage records -- which on 2026-09-15 found four with none, now settled.
      Eleven are walked whole, among them `AEGIS_Internals`, `014962-A00`,
      `008862-01`, `004977-02`, `008860-A03`, three handbook revisions and
      `009916-A00`; others are read on their own terms (`[CFG]`, `018901-A00`,
      `000792-A01`, the price list), the release notes and patents are triaged,
      and the rest are settled by a full-text hardware-term scan with every match
      read in context (`CONTENT_TRIAGE.md`). It explained the DS5500's
      zero-filled data section without a code change, corrected the DS5500's
      memory ceiling and a `RING.md` quotation, and left no chapter owed.
...
- [x] **The cartridge read loses a file's last short transfer — CLOSED
      2026-09-12; the restore reaches 401 entries and `Restore complete.` on
      both models.** Two defects, each found by instrumenting rather than
      reasoning, after five mechanisms were eliminated by measurement. **One**:
      the mark's byte crosses the host's bus only when the host's DMA discovers
      the mark, never when the card's read-ahead does — which removed
      `0028001E`. **Two**: `[SC499]` §1.11's parenthesis, "power-on reset …
      performs the same functions", runs one way, and this core read it
      backwards — RSTDMA reset the whole controller, discarding the EXCEPTION
      and `FIL` the host had not yet read, which is `(00280022)`.
      *Verification: 401 entries and `Restore complete.` on the DN3500 and the
...
- [x] **The DN5500's 68040 instruction core — closed 2026-09-15.** A DS5500
      went from two instructions and an F-line trap to Domain/OS: `CINV`/`CPUSH`
      and the 68040 `MOVEC` set, the I/O protection map and `MD14`, the MMU and
      ATCs joined with `PFLUSH`/`PTEST`, the self-test's cache status bit 4, the
      sealed configuration table and the clock, the SAU 14 install, then the
      caches attached (2026-09-13) and carrying fetches and operands
      (2026-09-15) — physically tagged lines, write-through and copyback, table
      searches through the data cache, and `CPUSH` writing dirty data back.
      `[RN104]` §4.12's four errata recorded as divergences. `PROVISIONAL`: SC1/SC0
      and TBI as `[040DH]` §7's reference adapter ties them, and §4.3.3's
      locked-hit re-enable sentence. *Verification: Domain/OS to `lcnode` and the
      SCSI round trip with `cacr 80008000`; `m68040_cache_access_suite`, 17
      tests; `tools/dn5500-identity.sh` `386D6B4E902E34A1`.* Detail in
      `PROJECT_STATUS.md`.
...
- [x] **A per-cycle processor** -- done 2026-09-08. `ap_machine_tick` advances
      the machine by exactly one machine cycle and `--cycle-stepped` drives a
      whole boot through it: the bus was already cycle-accurate, so what this
      needed was the sequencer's clock timeline and a loop that hands an
      instruction's cycles out one at a time. Everything outside the CPU -- bus,
      arbiter, devices, interrupts -- now sees a true per-cycle machine.
      *Verification, this phase's standard and not a weaker one: the 350 M
      identity boot is `FE2BB02AEF1F4624` **both** stepped by instruction and
      stepped by machine cycle, the reference golden unmoved; `ctest` 140/140
      both presets. Detail in `PROJECT_STATUS.md`; `FINDINGS.md` C254.*
      *The one approximation is named and measured*: the CPU's **internal** state
      commits at the instruction's start. The only observer that could tell is a
...
- [x] **A device consulted while the processor is stalled saw a stale time —
      landed 2026-09-09.** `ap_machine_tick`'s stall loop ticked the bus and
      counted the clock without advancing the board, so
      `ap_board_processor_may_run` and every DMA request line — both polled on
      every tick of the stall — were answered at the instant the stall began.
      The loop now carries its own instant and advances the board a CPU clock at
      a time; `ap_clock_duration` is `cycles * period`, so the step's single
      conversion still lands on the same instant and nothing is counted twice.
      **Behaviour-neutral on the reference boot, measured rather than argued**:
      identity `1AE206D37D8A8D1F` and the report byte-identical, which is what
      deadline-driven devices reaching the same instant by steps or by one jump
      should give. It unblocks the tape's byte rate, which is what found it.
...
- [x] **A resumable sequencer — CLOSED 2026-09-16, and it took no sequencer.**
  ...
  - [x] **`--legacy-instruction-bus` deleted, 2026-09-16**, and with it
        `cycle_bus`, `pending_cycles`, `defer_cycle_delivery`, `pending_rmc`,
        the pre-instruction stall, the instruction-wide `RMC` assertion and the
        `clock_events` replay. One schedule again, and `ap_machine_tick` is
        `ap_machine_run(.., 1)` -- the two loops the same code rather than two
        that agree. Both references move **for coverage only**: the two removed
        fields were hashed, so DN3500 becomes `DA70126426C9A937` and DS5500
        `D8B6F9244ACEF028` at *identical* clock counts, every other line of both
        reports byte-identical to the run before. *Verification: `ctest` 153/153
        on both presets; identities diffed line by line against their
...
- [x] **The stored device cursors — CLOSED 2026-09-16.** Six parts, not the four
      this item inherited: `ap_scsi` and `ap_wd7000` arrived with SCSI on
      2026-09-13 and nothing noticed, which was the item's real defect. All four
      board accessors now take the access's instant and advance the addressed
      device to it, so a cursor is current **by construction** rather than by a
      schedule's cadence — and a schedule may now skip advances without the
      hazard, which is what this was a prerequisite for. **259 call sites, not
      the 120 recorded**; every one found by the compiler. Both identities
      unchanged: on the current schedule the constructed instant and the
      cadenced one coincide, and the change is that they can no longer diverge.
      *Verification: `ctest` 153/153 on both presets; both identities re-run;
      the cadence test and the census both kept and both proved to fail.
      Detail in `PROJECT_STATUS.md`.*
...
**READ WHOLE 2026-08-20 — 330 of 330 pages.** Front matter, chapters 1 to 12,
both appendices, the index and the reader's response form. Coverage, page by
page and with what each yielded, is `docs/references/002398-04_WALK.md`; this
line is deliberately the only figure kept here, because two places holding the
same number is what let them diverge once already.
...
- [x] **`E0007` — closed 2026-09-09. The kernel now finds `bscom/rbak_shell`
      and runs it.** `RBAK_BS reloading system software from cartridge tape....`
      / `*** This program will replace system software on your disk.` — the
      install environment, which is this item's stated verification.
      **The defect: tape data leaves this card through `DACK` and through
      nothing else, and a *programmed* read of `BASE+0` was taking a byte too.**
      94 went out that way against 35,662,848 through `DACK`, and 94 is exactly
      the drift that left every block header 77 bytes out of place, so the
      kernel rejected each one — `Seq out of order: expected 1, read 46C70E00`,
      which is image block 23 at offset 77, byte for byte.
      **Settled by the oracle**, the documents having run out: `[SC499]`'s
      figures have the host poll *status* and never data across a transfer. Two
      narrower remedies were tried first and **both were refuted by the boot**.
      The run has an invocation at last: `tools/cartridge-boot.sh`.
      *Verification: `tape_suite` 29 → 30, `qic_suite` 29 → 30; identity
      `7F793C44586F263B` **unchanged**.* Detail in `PROJECT_STATUS.md`.
...
- [x] **Implement the five floppy commands this core rejects — done
      2026-08-22.** `02` READ A TRACK, `05` WRITE DATA, `09` WRITE DELETED
      DATA, `0A` READ ID, `0C` READ DELETED DATA, from `[765]` pp. 8-9 and
      13-14 with every opcode cross-checked against `[8272A]` Table 4. **The
      floppy can be written.** A test asserted the defect and had to be
      replaced — `test_an_unlisted_command_including_write_data_is_invalid`
      encoded the same misreading as the code, which is the failure mode
      `CLAUDE.md` names. Two behaviours fall out of the image format rather
      than being chosen: `0C` always reports `CM` because every `.afd` address
      mark is normal, and `09` writes the data but cannot lay the mark down.
      Both named in `ap_omti.h` with the cost to close.
      *Verification: `afd_suite` 40 → 46 — a WRITE DATA read back through
      READ DATA rather than out of the buffer, write-protect refusing before
      the data phase, READ ID's C/H/R/N, `CM` on a normal mark, READ A TRACK
