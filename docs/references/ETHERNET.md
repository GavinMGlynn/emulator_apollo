...
| 17 | **The oracle confirms all three of finding 12's numbers, independently and from a different kind of source.** MAME's Apollo driver configures its 3c505 slot with `DEVICE_INPUT_DEFAULTS("IO_BASE", 0x3f0, 0x300)`, `("IRQ_DRQ", 0x0f, 0x0a)` and `("IRQ_DRQ", 0x70, 0x60)` -- I/O `300`, **IRQ 10**, **DRQ 6**, its own comments saying so -- plus `ROM_OPTS` enabling the host ROM. That is `[S3K]` Figure 14-3's jumper drawing agreed with by a driver written from different sources, and it arrived *after* the page-image reading rather than informing it, which is what makes it a check and not a fit | `ext/mame/src/mame/apollo/apollo_m.cpp:1039-1052` | confirmed |
| 17a | **And it corroborates finding 13's harder half, the one that inverts.** MAME drives the host DMA request as `update_isa_drq((m_hsr & HSR_HRDY) && (hcr & HCR_DMAE))` -- `HRDY` and the enable, with `ARDY` nowhere in it. Two independent readings of §1.9.4's truth table taking the same column, on the point where taking the other one would drive the channel exactly when the hardware does not | `ext/mame/…/3c505.cpp:640` | confirmed |
| 17b | **What the oracle does *not* share is the architecture, and this is the open design decision answered from the other side.** MAME instantiates `I80186(config, m_cpu, 16_MHz_XTAL)` and loads the adapter's real firmware, `0729-12_a.3h` and `0729-62_a.3f` -- so on that machine the mailbox's behaviour is *emergent* from the card's own 80186, where ours implements the PCB protocol host-side. Its device reset sets `m_hsr = HSR_HCRE` with the `ASF` bits clear; the `11` -> `00` power-on finding 16b recovered is produced there by firmware executing, not by a device model. **So our `adapter_initialising` is a host-side stand-in for firmware the oracle actually runs**, and is marked `PROVISIONAL` in `ap_3c505.h` with that as its cost to close: the two adapter ROM dumps, which this project does not hold, and an 80186 | `ext/mame/…/3c505.cpp:36-53, 217, 300` | confirmed |
| 17c | Which also bounds what a behavioural oracle diff can mean here. Comparing a host-side protocol against an emulated firmware is not the field-by-field comparison `dsp3500`'s CPU diff is; the honest comparison is **at the host interface** -- the register traffic the option ROM's self-test produces, which both machines run. Recorded so the diff is not later reported as narrower than it is | — | open |

| 18 | **The host-interface oracle diff is done, and it agrees -- at the instruction level.** Tapping our own `058006` through a boot with the option ROM fitted gives **22 writes and nothing else**: twenty alternating `00`/`10` from PCs `00080382` and `00080392`, then `C0` from `000803C2` and `00` from `000803C8`. Every one of those four PCs is `entry_05`'s own code at ROM base `080000` -- `$382`/`$392`'s `bclr`/`bset` of bit 4 and `$3C2`/`$3C8`'s hard reset -- so the trace and the disassembly close on each other. And 22 is exactly the write count the run's region census reports independently | `--boot-watch-write 58006`, `--3c505-rom` | confirmed |
| 18a | **Which is the same traffic, from the same instruction addresses, that finding 10a measured on the *oracle* before this model existed.** 10a recorded "all of it comes from PC `0008xxxx`", a read-modify-write at `+6` alternately clearing and setting **bit 4**, with `+2` answering `C0` and `50` -- PCs `080382`-`080398`. Ours now produces the identical sequence from the identical addresses, and 10c's decode of those two response bytes is already held up by a test. That is the host-interface comparison this item's verification asks for, and it passes: two machines running the same option ROM through the same instructions and agreeing on what the card does | finding 10a against 18 | confirmed |
| 18b | **A fresh MAME run adds a fact about the oracle rather than a disagreement.** Tapping `058000-05800F` on `dn3500` for 40 emulated seconds under the current (APOLLO_XXL-modified) checkout gives **zero** writes -- with the tap proven live in the same run by **21** writes to the `sio1` control range. So that machine does not exercise its 3c505 on this configuration; it is not a case of the two models differing about the card. Recorded because a zero from a tap is the reading this file's own rule says looks identical to a wrong range, and the control is what separates them | fresh `writetrace.lua` run | confirmed |
| 18c | **CORRECTION to finding 17b: the adapter's 80186 firmware dumps are on disk.** `tools/mame-oracle/out/roms/3c505/` holds `0729-12_a.3h`, `0729-62_a.3f`, the host option ROM and both address PROMs. 17b said "two dumps this project does not hold" and that was simply not checked. It changes the cost of closing the `adapter_initialising` approximation from *acquire ROMs* to *emulate an 80186*, which is a different and much better-defined piece of work | `ls tools/mame-oracle/out/roms/3c505/` | confirmed |
