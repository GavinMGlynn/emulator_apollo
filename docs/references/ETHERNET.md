...
| 15a | **Which is the good outcome, and it is the ring's situation one card along.** `CLAUDE.md` calls a controller's own firmware self-test "the hardware's test suite, for free", and this is the second one this project has got running. The item stops being "implement more of the card and hope" and becomes a numbered loop: the PROM tests the card, names its verdict on the console, and stops at an instruction. What it does *not* yet give is a subtest number the way the ring ROM's `E00000xx` codes do -- finding out whether this test reports one is the next step, and `000083C4` is where to read | finding 15 | open |
