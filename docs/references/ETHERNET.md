...
| 10b | What 10a does **not** establish: which named flag each of those positions is. `[DEV]` §1.9 names eleven flags and gives no positions, and a handshake showing "bit 4 of the control register moves bits 7 and 4 of the status register" is a *relationship*, not an assignment. Two candidate readings fit it and the measurement does not choose between them, so none is recorded. **This is the honest halfway point of open question A**: the traffic is now observable and reproducible, and turning it into positions needs the option ROM disassembled or more of the sequence | — | open |

## Open

| # | Question | How it will be answered |
| --- | --- | --- |
| A | Bit positions for the eleven named flags in the host status and control registers | `[HIS]` ch. 2 if it can be found; otherwise the oracle, per finding 8a. **Started**: findings 10-10b measure the option ROM's probe handshake, which relates `+6` bit 4 to `+2` bits 7 and 4 without yet naming any of them |
