...
| 12b | **The likely reason is question C, not the device.** This is a **DN4500** board's ROM running on a **DN3500** machine and map, because `identity-boot.sh` is the DN3500's. A PROM that scans, accepts and calls a ROM for hardware its own machine does not otherwise have is not obliged to survive it. Distinguishing "our device is wrong" from "this ROM does not belong on this machine" needs `4500_BOOT_13167_02_MD7R.0.32.bin` and a DN4500 map, which is what question C has said all along | 3, 12a | open |
...
| --- | --- | --- |
| A | Which of `$D40000`, `$D80000` and `$DA0000` is the frame buffer, and its geometry | **None of them, on this evidence** — all three are ports written by fixed-address instructions, and finding 8's longword path at `$D80008` with a ready bit is the closest thing to a memory route. `[CFG]`'s Series 4500 display options bound the resolutions, and finding 4a's table already carries `00000400` = 1024 |
