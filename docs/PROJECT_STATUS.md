...
## Phase 5's framebuffer, from a black screen to Domain/OS on the glass

*Moved verbatim from `COMPLETION_PLAN.md`, where this had grown to a single
**983-line** open item -- the longest in the file and unreadable as a plan.
`tools/check_docs.py` never caught it because it only length-checks
*completed* items. The investigation below is finished and its conclusion is
that the display works; it is kept because most of it appears nowhere else.*
