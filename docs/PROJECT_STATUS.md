...
| 68882 transcendental functions | **Not implemented.** `FSIN`, `FCOS`, `FTAN`, `FATAN`, `FETOX`, `FLOGN` and the rest report unimplemented rather than approximating | §4.3.2 is explicit that there is nothing to transcribe: "the IEEE specification does not define the error bound to which transcendental (**except square root**) functions are to be performed", and Motorola publishes no algorithm -- only bounds. "The worst-case accuracy of any transcendental function is one unit in the last place of double precision (which is equal to 4096 units in the last place of extended precision). The typical error bound ... is approximately 64 units in the last place of extended precision." So a correctly-rounded implementation would be *more* accurate than the part and would differ from it in the low bits of almost every result -- which for a reference core is a divergence, not an improvement. Reporting unimplemented keeps that visible; approximating would hide it | Either recover the algorithms from a 68881 ROM disassembly, or accept a correctly-rounded implementation as a **named divergence class** with the published bound as its size. **Two candidate shortcuts were investigated and neither closes it.** Motorola's `M68040FPSP` is sometimes described as implementing the same algorithms as the 68881/68882 in software, which would make its source the missing algorithm -- but Appendix E says only that "the error bound is **equivalent** to that of the MC68881/MC68882". An equivalent bound is not an identical result: two implementations can meet the same bound and differ in the low bits of almost every value, which is exactly this divergence. So the FPSP would give a bit-exact model of the *68040's software emulation* and not of the *68882's hardware*. The `68881-fpga` VHDL project (MIT, so no licence obstacle) likewise implements its own "BRAM-based seed tables with Taylor/CORDIC iteration" and validates functionally with Whetstone and Mandelbrot rather than by bit comparison; its own FPSP checklist records that it uses Taylor coefficients where the FPSP uses minimax, so it is a third implementation of the same bound. **A third lead settles what the field actually does.** `Amiberry`, and `WinUAE` before it, inherit their floating-point unit from `Previous`, the NeXT emulator, whose `softfloat_fpsp.cpp` states its provenance outright: "algorithms for transcendental functions supported by MC68881 and MC68882 mathematical coprocessors. The functions are **derived from FPSP library**." So the most accuracy-minded 68k emulation lineage implements the 68882's transcendentals from Motorola's own software package rather than from the hardware -- which is the best available practice and still not bit-exactness, for the reason Appendix E gives. That **improves the second route**: rather than a fresh correctly-rounded implementation, implement the FPSP algorithms from Motorola's published source, which was written to match the part and is what every serious emulator uses. The licence caveat is the familiar one -- `Amiberry` is GPL-3.0 and this core is MIT, so it may be read for facts exactly as `ext/musashi` is and never copied from. Two further leads were checked and are not relevant: `MotoFFP` implements Motorola's *Fast Floating Point*, a 32-bit non-IEEE format, and names no coprocessor; and `SoftIEEE` is an independent software library. `SoftIEEE` is worth citing anyway, because it corroborates this entry's central claim from outside Motorola: it reports 0.5 ulp for the IEEE-specified operations, "identical to what the 68881/882 deliver", and then says its transcendentals are "**much better than the hardware implementation in the 68881/882 coprocessors which only offers double precision for such functions**". So a careful software implementation is *more* accurate than the part -- which is exactly why implementing one here would be a divergence rather than an improvement. Five leads investigated in all; none achieves or claims bit-exact agreement with the hardware. The second is cheap and the first is the only one that reproduces the hardware's bits. Affects only the transcendentals; every IEEE-specified operation, square root included, is exact |
...
| 68851 `U` and `M` write-back | **Not implemented.** A table search does not write the accessed and modified bits back into the descriptors it read. The bits are carried into the ATC entry, and `PLOADW` marks its entry modified, but the tables in memory are left unchanged | The bits exist for the operating system's page-replacement policy, not for translation: no address translates differently because a `U` bit is stale. Domain/OS reads them when choosing a victim page, so a long-running system would make worse eviction choices -- a performance and behaviour difference that appears only under memory pressure, never in a single translation | Write `U` back on every search that reaches a page descriptor and `M` on a permitted write, which needs the search to hold the descriptor addresses it visited. Cheap once the search returns a path rather than a result; do it before any paging workload is measured |
