...
| 68851 `U` and `M` write-back | **Implemented.** A search now returns the path of descriptors it read, and `ap_m68851_status_writes` produces the byte cycles §5.1.5.3.11's table calls for. The status byte is the fourth of the descriptor in both formats -- `U` is bit 35 of a long descriptor and bit 3 of a short one, `M` bit 36 and bit 4, and in each case that is bit 3 and bit 4 of `address + 3`, which is why the manual can speak of a single "descriptor status byte" | No longer provisional. Three readings were needed and each is sourced. **A descriptor already carrying the right bits produces no cycle at all**: the part "optimizes its activity by examining the U and M bits in descriptors as they are fetched, and only performing write cycles to modify these bits are required". **The cycle type is part of the specification**: a read-modify-write "whenever it is required to set the used bit but not affect the state of the modified bit", so two MMUs sharing a tree cannot lose each other's `M`, and a plain write only when the same cycle sets `M` too; pointer descriptors, which "do not contain modified bits, are not referenced using read-modify-write sequences". And **the path survives a fault** -- "note that a pointer may be fetched, and its U bit set, for an address to which access is denied at another level of the tree" -- while the descriptor that *causes* the denial is not marked, since nothing translated through it | Closed. The remaining judgement is that last one: the manual states the pointers above a denial are marked and is silent on the invalid descriptor itself, so not marking it is a reading rather than a quotation. It is the containable one -- `U` exists for the operating system's page-replacement policy, and an invalid descriptor has no page to replace |
