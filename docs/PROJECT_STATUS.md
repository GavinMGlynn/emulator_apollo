...
| 68030 published timings (§11.6) | 59 rows from §11.6.6, §11.6.8, §11.6.9, §11.6.11, §11.6.12, §11.6.15 and §11.6.16, the branches reached through their run-time outcome rather than by opcode. `LSR` and `NOP` both agree with the oracle's independent measurements, checked against **both** published columns on a running machine, the pure-microcode ones; the four divides carry the manual's data-dependent marker and are `PROVISIONAL`. Scheduled into the step's clock as `max(microcode, bus)`, since the tables show prefetch overlaps execution | `timing_table_suite`, 7 tests |
