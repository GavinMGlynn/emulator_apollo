...
| 68030 published timings (§11.6) | 43 rows from §11.6.6, §11.6.8, §11.6.9, §11.6.11 and §11.6.12 — register, memory-destination, MOVE, single-operand and immediate-count shift forms. `LSR` agrees with the oracle's independent measurement, checked against **both** published columns on a running machine, the pure-microcode ones; the four divides carry the manual's data-dependent marker and are `PROVISIONAL`. Scheduled into the step's clock as `max(microcode, bus)`, since the tables show prefetch overlaps execution | `timing_table_suite`, 7 tests |
